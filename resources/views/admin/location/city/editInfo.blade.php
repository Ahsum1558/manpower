...
@include('admin.includes.alert')
                <form action="{{ route('admin.city.updateInfo', ['id'=>$city_data_info[0]->id]) }}" class="form-group" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="row">
                    <!--Tab slider End-->
                    <div class="col">
                            ...
                            <div class="new-arrival-content pr">
                                ...
                                <div class="row mb-2">
                                    ...
                                    <div class="col-9 mybtn">
                                        ...
                                        <a class="btn submitbtn mb-2 form-control inline_setup text-uppercase pull-right" href="{{ route('admin.city.show', ['id'=>$city_data_info[0]->id]) }}">Back</a>
                                    </div>
                                </div>
                            </div>
                        </div>
...
<script>
    $(document).ready(function(){
        $('#select').change(function() {
            ...
            if(countryId){
                $('#overlay .loader').show();
                $.get("{{ route('admin.city.get') }}", {country_id:countryId}, function(data){
                    $('#overlay .loader').hide();
                    console.log(data);
        ...
        $('#single-select').change(function() {
            ...
            if(divisionId && countryId){
                $('#overlay .loader').show();
                $.get("{{ route('admin.city.getDistrict') }}", {division_id:divisionId, country_id:countryId}, function(data){
                    $('#overlay .loader').hide();
                    console.log(data);
