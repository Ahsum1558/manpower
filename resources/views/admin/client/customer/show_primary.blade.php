    ...
    <div class="card card_line">
        ...
        @if(Auth::check() && (Auth::user()->role == 'admin' || Auth::user()->role == 'author'))
            @if($customer_single_data[0]->rateValue == 0)
                <a class="btn submitbtn mb-2 form-control inline_setup text-uppercase" href="{{ route('admin.customer.rateAdd', ['id'=>$customer_single_data[0]->id]) }}">Add Working Rate</a>
            @endif
        @endif
            @if($customer_single_data[0]->value == 0)
                <a class="btn submitbtn mb-2 form-control inline_setup text-uppercase" href="{{ route('admin.customer.documentAdd', ['id'=>$customer_single_data[0]->id]) }}">Add Documents</a>
            @else
